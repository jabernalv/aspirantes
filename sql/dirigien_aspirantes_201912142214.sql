...
--
-- Servidor: localhost
-- Tiempo de generación: 15-12-2019 a las 03:13:46
...
INSERT INTO `estado_aspirante_proceso` (`id`, `nombre`, `activo`) VALUES
(1, 'En proceso', 1),
(2, 'Admitido', 1),
(3, 'No admitido', 1);
...
INSERT INTO `proceso` (`id`, `entidad_id`, `nombre`, `activo`) VALUES
(1, 1, 'Proceso 1', 1);
...
CREATE TABLE `soporte` (
  ...
  `comentarios_analista` varchar(255) NOT NULL COMMENT 'Comentarios del analista',
...
ALTER TABLE `soporte`
  ADD PRIMARY KEY (`uuid`),
  ADD UNIQUE KEY `UQ_SOPORTE_PROCESO_ASPIRANTE_MD5` (`proceso_id`,`aspirante_uuid`,`md5`) USING BTREE,
  ADD UNIQUE KEY `UQ_SOPORTE_RUTAWEB` (`archivo_ruta_web`),
  ADD KEY `IX_SOPORTE_TIPOSOPORTE` (`tipo_soporte_id`),
  ADD KEY `IX_SOPORTE_PROCESO` (`proceso_id`),
...
ALTER TABLE `soporte`
  ...
  ADD CONSTRAINT `FK_SOPORTE_PROCESO` FOREIGN KEY (`proceso_id`) REFERENCES `proceso` (`id`) ON UPDATE CASCADE,
